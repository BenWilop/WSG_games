...
model = move_to_and_update_config(model.to(device0), device1)  # <-- cuda:0 to cuda:1
x = torch.zeros((1, 1), dtype=torch.long, device=device1)

def print_device_hook(activation, hook):
    print(f"Hook '{activation.name}': \tDevice={repr(activation.device)}")
    return activation

fwd_hooks = [
    ("hook_embed", print_device_hook),
    ("hook_pos_embed", print_device_hook),
...
# Moving model to device:  cuda
# Hook 'None':    Device=device(type='cuda', index=1)
# Hook 'None':    Device=device(type='cuda', index=1)
# Hook 'None':    Device=device(type='cuda', index=0)  # <-- Should be cuda:1 (?)
