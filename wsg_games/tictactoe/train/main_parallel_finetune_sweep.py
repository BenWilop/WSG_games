...
def worker(
    ...
    while True:
        ...
        if load_finetuned_model_get_matching_files(
            ...
            print(
                f"[Worker {gpu_id}] already finetuned {weak_size}→{strong_size}, skipping"
            )
            continue
        ...
        if weak_model is None or strong_model is None:
            print(
                f"[Worker {gpu_id}] missing pretrained model for {weak_size} or {strong_size}, skipping"
            )
            continue
        ...
        # save
        save_model(
            finetuned_model, run_id, finetuned_project_name, exp_name, experiment_folder
        )
...
def main_finetune():
    pretrained_project_name = (
        "tictactoe_pretrained_reverse_rule_no_overlap_split_start_third_200k"
    )
