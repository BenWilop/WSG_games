...
def finetune_task(pretrained_project_name: str,
                  finetuned_project_name: str,
                  experiment_folder: str,
                  weak_finetune_data, val_data, test_data,
                  training_cfg: dict,
                  weak_size: str, strong_size: str,
                  gpu_pool):

    gpu_id = gpu_pool.get()
    try:
        t.cuda.set_device(gpu_id)
        print(f"[GPU {gpu_id}] Starting task: {weak_size} -> {strong_size}")

        if load_finetuned_model_get_matching_files(finetuned_project_name, weak_size, strong_size, experiment_folder):
            print(f"[GPU {gpu_id}] Finetuned {strong_size} to {weak_size} exists. Skipping ...")
            return None

        weak_model = load_model(pretrained_project_name, weak_size, Goal.WEAK_GOAL, experiment_folder)
        if not weak_model:
            print(f"[GPU {gpu_id}] Weak model of size {weak_size} not found, skipping.")
            return None

        strong_model = load_model(pretrained_project_name, strong_size, Goal.STRONG_GOAL, experiment_folder)
        if not strong_model:
            print(f"[GPU {gpu_id}] Strong model of size {strong_size} not found, skipping.")
            return None

        finetuned_model = deepcopy(strong_model)
        print(f"[GPU {gpu_id}] Finetuning: {weak_size} -> {strong_size}")
        ...
        save_model(finetuned_model, run_id, finetuned_project_name, experiment_name, experiment_folder)
        return (weak_size, strong_size, run_id, gpu_id)
    finally:
        t.cuda.empty_cache()
        gpu_pool.put(gpu_id)


def finetune_sweep_parallel(pretrained_project_name: str,
                            finetuned_project_name: str,
                            experiment_folder: str,
                            weak_finetune_data, val_data, test_data,
                            training_cfg: dict,
                            num_gpus: int):
    model_sizes = ["nano", "micro", "mini", "small", "medium", "large", "huge"]
    tasks = [(weak_size, model_sizes[j])
             for i, weak_size in enumerate(model_sizes)
             for j in range(i + 1, len(model_sizes))]
    
    print(f"Total tasks: {len(tasks)}. Using {num_gpus} GPUs dynamically.")
        ...
        with ProcessPoolExecutor(max_workers=len(tasks)) as executor:
            future_to_task = {
                executor.submit(
                    ...
                    weak_finetune_data, val_data, test_data,
                    training_cfg,
                    weak_size, strong_size,
                    gpu_pool
                ): (weak_size, strong_size)
                for weak_size, strong_size in tasks
            }
            for future in as_completed(future_to_task):
                weak_size, strong_size = future_to_task[future]
                try:
                    result = future.result()
                    if result:
                        print(f"[GPU {result[3]}] Task {result[0]} -> {result[1]} finished (run id: {result[2]}).")
                except Exception as exc:
                    print(f"Task {weak_size} -> {strong_size} generated an exception: {exc}")
